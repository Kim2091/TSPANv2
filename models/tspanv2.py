"""
TSPANv2: Improved TSPAN, with an enhanced temporal processing system
Based on SPAN (Swift Parameter-free Attention Network) adapted for VSR using TSCUNet's temporal processing
"""
...
class Conv3XC(nn.Module):
    ...
    def __init__(self, c_in, c_out, gain1=1, gain2=0, s=1, bias=True, relu=False):
        ...
        self.sk = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=1, 
                           padding=0, stride=s, bias=bias)
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=c_in, out_channels=c_in * gain, kernel_size=1, 
                     padding=0, bias=bias),
            nn.Conv2d(in_channels=c_in * gain, out_channels=c_out * gain, kernel_size=3, 
                     stride=s, padding=0, bias=bias),
            nn.Conv2d(in_channels=c_out * gain, out_channels=c_out, kernel_size=1, 
                     padding=0, bias=bias),
        )

        self.eval_conv = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=3, 
                                   padding=1, stride=s, bias=bias)
...
class TemporalSPANBlock(nn.Module):
    ...
    """
    def __init__(self, in_nc=3, out_nc=3, dim=48, num_blocks=6, bias=False, use_checkpoint=False):
        super(TemporalSPANBlock, self).__init__()
        ...
        self.blocks = nn.ModuleList([
            SPAB(dim, bias=bias) for _ in range(num_blocks)
        ])
...
class Vsrspan(nn.Module):
    """
    VSR-SPAN: Video Super-Resolution using SPAN architecture
    ...
    def __init__(self, 
        ...
        super(Vsrspan, self).__init__()
        
        if clip_size % 2 == 0:
            raise ValueError('VSR-SPAN clip_size must be odd')
    ...
    def forward(self, x):
        ...
        x = nn.ReflectionPad2d((paddingLeft, paddingRight, paddingTop, paddingBottom))(x)
        x = self.m_head(x)  # (B*T, dim, H+pad, W+pad)
        x = x.view(b, t, self.dim, h + paddingH, w + paddingW)  # (B, T, dim, H+pad, W+pad)
        ...
        for layer in self.m_layers:
            ...
            out = torch.empty(b, num_windows, self.dim, h + paddingH, w + paddingW,
                            device=x.device, dtype=x.dtype)
            
            # Slide 3-frame window
            for i in range(num_windows):
                # Concatenate 3 consecutive frames: [i, i+1, i+2]
                x_window = x[:, i:i+3, ...].reshape(b, -1, h + paddingH, w + paddingW)
        ...
        # Remove padding
        x = x[..., 
              paddingTop * self.upscale : paddingTop * self.upscale + h * self.upscale,
              paddingLeft * self.upscale : paddingLeft * self.upscale + w * self.upscale]
